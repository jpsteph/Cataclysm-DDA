                       ...
                       bool /*debug*/ ) const
{
    ...
    if( !contents.empty() && parts->test( iteminfo_parts::DESCRIPTION_CONTENTS ) ) {
        ...
        for( const item *contents_item : contents.all_items_top( item_pocket::pocket_type::CONTAINER ) ) {
...
std::string item::tname( unsigned int quantity, bool with_prefix, unsigned int truncate ) const
{
    ...
    if( is_corpse() || typeId() == "blood" || item_vars.find( "name" ) != item_vars.end() ) {
        ...
        const item &contents_item = *contents.all_items_top( item_pocket::pocket_type::CONTAINER ).front();
...
{
    return has_item_with( []( const item & food ) {
        return food.is_food();
    } ) ||
